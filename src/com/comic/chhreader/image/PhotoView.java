...
/**
 * This class extends the standard Android ImageView View class with some
 * features that are useful for downloading, decoding, and displaying Picasa
 * images.
 * 
 */
public class PhotoView extends ImageView {

	private boolean mLoadNetImage = false;
	 ...
	 * reference prevents memory leaks and crashes, because it automatically
	 * tracks the "state" of the variable it backs. If the reference becomes
	 * invalid, the weak reference is garbage- collected. This technique is
	 * important for referring to objects that are part of a component
	 * lifecycle. Using a hard reference may cause memory leaks as the value
	 * continues to change; even worse, it can cause crashes if the underlying
	 * component is destroyed. Using a weak reference to a View ensures that the
	 * reference is more transitory in nature.
	 */
	...
	private int mImageDownloadingId = R.drawable.gray_image_downloading;
	private int mImageFailId = R.drawable.gray_image_downloading;
	 ...
	 * 
	 * @param context
	 *            A Context to use with the View
	 * @param attributeSet
	...
	/**
	 * Creates an ImageDownloadView, gets attribute values, and applies a
	 * default style
	 * 
	 * @param context
	 *            A context for the View
	 ...
	 *            The entire set of attributes for the View
	 * @param defaultStyle
	 *            The default style to use with the View
	 */
	 ...
	 * 
	 * @param visState
	 *            The visibility state (see View.setVisibility)
	 */
	...
	public void setAsPreview(boolean value) {
		mAsPreview = value;
	}

	public void setCustomDownloadingImage(int id) {
		mImageDownloadingId = id;
	}

	public int getCustomDownloadingImage() {
		return mImageDownloadingId;
	}

	public void setCustomFailImage(int id) {
		mImageFailId = id;
	}

	public int getCustomFailImage() {
		return mImageFailId;
	}

	/*
	 * This callback is invoked when the system attaches the ImageView to a
	 * Window. The callback is invoked before onDraw(), but may be invoked after
	 * onMeasure()
	 */
	@Override
	...
	/*
	 * This callback is invoked when the ImageView is removed from a Window. It
	 * "unsets" variables to prevent memory leaks.
	 */
	@Override
	...
	/*
	 * This callback is invoked when the system tells the View to draw itself.
	 * If the View isn't already drawn, and its URL isn't null, it invokes a
	 * Thread to download the image. Otherwise, it simply passes the existing
	 * Canvas to the super method
	 */
	@Override
	protected void onDraw(Canvas canvas) {
		...
		if ((!mIsDrawn) && (mImageURL != null)) {
			...
			Loge.d("PhotoView mLoadNetImage: " + mLoadNetImage);
	...
	/**
	 * Sets the current View weak reference to be the incoming View. See the
	 * definition of mThisView
	 * 
	 * @param view
	 *            the View to use as the new WeakReference
	 */
	...
	public void setImageDrawable(Drawable drawable) {
		...
		/*
		 * Sets the View state to visible if the method is called with a null
		 * argument (the image is being cleared). Otherwise, sets the View state
		 * to invisible before refreshing it.
		 */
		if (drawable == null) {
	...
	/**
	 * Attempts to set the picture URL for this ImageView and then download the
	 * picture.
	 * <p>
	 * If the picture URL for this view is already set, and the input URL is not
	 * the same as the stored URL, then the picture has moved and any existing
	 * downloads are stopped.
	 * <p>
	 * If the input URL is the same as the stored URL, then nothing needs to be
	 * done.
	 * <p>
	 * If the stored URL is null, then this method starts a download and decode
	 * of the picture
	 * 
	 * @param pictureURL
	 *            An incoming URL for a Picasa picture
	 * @param cacheFlag
	 *            Whether to use caching when doing downloading and decoding
	 * @param imageDrawable
	 *            The Drawable to use for this ImageView
	 */
	public void setImageURL(URL pictureURL, boolean cacheFlag, boolean asPreview, boolean loadNetImage,
		...
		if ((mIsDrawn) && (pictureURL != null)) {
			...
			/*
			 * Starts a download of the picture file. Notice that if caching is
			 * on, the picture file's contents may be taken from the cache.
			 */
	 ...
	 * 
	 * @param drawable
	 *            A Drawable to use for the ImageView
	 */
